...
The Claim Verifying Agent ensures the `credibility of the content` by systematically extracting facts & claims mentioned in the YouTube video and verifying their accuracy using real-time web search & Fact Check API Tools.
...
- This agent answers the most critical question: `Is this information actually true?` High production quality no longer guarantees accuracy; polished videos may contain AI-invented "facts" that take hours to disprove manually.

- Instead of gambling on accuracy, this tool automates professional fact-checking, delivering results in sub 30 seconds. By verifying content before you watch, it prevents `misinformation from taking root` in the first place, therefore combating fast-spreading "fake news".

---

## Purpose

The purpose of this agent is to automate fact-checking and providing results about the `credibility` of the content. Specifically, this agent -

1. `Prevents misinformation consumption`: Acts as a pre-watch filter against AI hallucinations by flagging false claims before you view.

2. `Provides transparent evidence`: Beyond simple labels, it details exactly which claims are verified or contradicted, empowering decisions based on proof.

3. `Enables crisis-time verification`: During emergencies, it verifies facts in 30 seconds—outpacing human checkers to separate truth from panic.
...
- #### How it works here:
  ...
  1.  `Context Analysis:` First, we analyze the video's metadata (title, description, channel info, transcript) to determine its domain, tone, and audience
...
The claim verifying agent works with an LLM as well as additional tools to first extract the [Relevant Claims](/docs/agents/claim-verifying-agent#relevant-claim) from the provided transcript, verify these claims, generate a report and pass that on to the supervisor agent.
...
- Strategy A (Direct Web Search): If `verificationStrategy` is `WEB_SEARCH`, go directly to Brave Search

- Strategy B (Fact Check with Fallback): If `verificationStrategy` is `FACT_CHECK_API`:
...
Task 3.1: Execute Brave Search queries and filter the results
...
```ts filename="" copy
// 1. Gather raw search results
const rawResults = await Promise.all(
  claim.braveSearchQueries.map((q) => executeBraveSearch(q))
);

// 2. Deduplicate and filter for high-quality domains (.gov, .edu, major news)
const evidenceSnippets = filterAndRankSnippets(rawResults);
```

Task 3.2: LLM Consensus Analysis (The New Brain - Agentic RAG Approach)

We cannot rely on simple keyword matching. We pass the Original Claim and the Evidence Snippets to a lightweight LLM (e.g., GPT-4o-mini) to determine the truth.

```ts filename="/pass-to-lightweight-llm.ts" copy
{
  "claim": "Eating dirt cures immunity",
  "evidence": [
    { "source": "Mayo Clinic", "snippet": "There is no scientific evidence supporting soil ingestion..." },
    { "source": "CDC", "snippet": "Ingesting soil can lead to parasitic infections..." }
  ]
}
```
...
"You are a strict fact-checker. Compare the CLAIM against the provided EVIDENCE SNIPPETS.
Determine if the evidence SUPPORTS, CONTRADICTS, or is INCONCLUSIVE regarding the claim.
Return a verdict, a confidence score (0-1), and a one-sentence reasoning."

Task 3.3: Parse verdict

```ts filename="parse-verdict.ts" copy
async function verifyWithBraveSearchOnly(claim: ClaimObject) {
  // 1. Search
  const snippets = await gatherEvidence(claim.braveSearchQueries);
  ...
  const analysis = await llm.generate({
    messages: [
      {
        role: "system",
        content: "Analyze this claim against these search snippets...",
      },
      {
        role: "user",
        content: JSON.stringify({ claim: claim.statement, snippets }),
      },
    ],
  });
  ...
  return {
    claimId: claim.id,
    verdict: analysis.verdict, // "TRUE" | "FALSE" | "UNVERIFIED"
    confidence: analysis.confidence,
    reasoning: analysis.reasoning,
    evidence: snippets.slice(0, 3), // Keep top 3 sources for the UI
  };
}
```

**Step 3:** Path B - Fact Check with Fallback

For claims with `FACT_CHECK_API` strategy. Google Fact Check returns structured data (`textualRating`). We can map it algorithmically to save tokens, but we will still need to use an LLM to generate reasoning.

Task 3.1: Query & Map

Google returns ratings like "False" or "Mostly Fake." We map these to our standard format.

| Google Rating                       | SlopScore Verdict                  |
| :---------------------------------- | :--------------------------------- |
| "True", "Correct"                   | `TRUE`                             |
| "False", "Incorrect", "Mostly Fake" | `FALSE`                            |
| "Mostly False", "Misleading"        | `FALSE`                            |
| "Half True", "Missing Context"      | `UNVERIFIED` (or Trigger Fallback) |

```ts filename="/fact-checl-logic.ts" copy
async function verifyWithFactCheckAndFallback(claim: ClaimObject) {
  const result = await queryGoogleFactCheck(claim.factCheckQuery);

  if (result && result.claims && result.claims.length > 0) {
    const review = result.claims[0].claimReview[0];

    // Algorithmic Mapping (Fast)
    const verdict = mapRatingToVerdict(review.textualRating);

    if (verdict !== "UNVERIFIED") {
      return {
        claimId: claim.id,
        verdict: verdict,
        confidence: 1.0, // Fact checks are high confidence
        reasoning: `Fact-checked by ${review.publisher.name}: ${review.title}`,
        evidence: [{ source: review.publisher.name, url: review.url }],
      };
    }
  }

  // Fallback: If Google result is empty or ambiguous ("Half True")
  return await verifyWithBraveSearchOnly(claim);
}
```
...
**Step 1:** Assign verification status

For every claim, we assign a final status based on the evidence found.

| Verdict         | Condition                                                                                                              |
| :-------------- | :--------------------------------------------------------------------------------------------------------------------- |
| `TRUE`          | Supported by ≥2 authoritative sources. No contradictions.                                                              |
| `FALSE`         | Contradicted by ≥1 high-authority source (e.g., Reuters, NASA).                                                        |
| `UNVERIFIED`    | No reliable evidence found, or conflicting data.                                                                       |
| `HALLUCINATION` | The video attributes the claim to a source (e.g., "NASA says..."), but the source does not exist or says the opposite. |

**Step 2:** Format data and pass to Supervisor agent

This marks the end of the worflow for Claim Verifying agent.

---

## Agent's Final Output

This is the `JSON object` **returned to the Supervisor Agent**.

```ts filename="/final-output-claim-verifying-agent" copy
interface ClaimAgentOutput {
  agentName: "ClaimVerifyingAgent";
  meta: {
    totalClaims: number;
  ...
  summary: {
    ...
    hallucinatedCount: number; // High penalty
  };
  claims: Array<{
    id: number;
    text: string;
    verdict: "TRUE" | "FALSE" | "UNVERIFIED" | "HALLUCINATED";
    confidenceScore: number; // 0-1
    reasoning: string; // e.g., "Contradicted by official WHO report."
    evidence: Array<{
      sourceName: string; // e.g., "Reuters"
      url: string;
      snippet: string;
